...
def get_file(x: int):
    f = quantize(x)
    output_file = list(map(lambda y: FILES.index(y) + 1, FILES))
...
def get_rank(x: int):
    r = quantize(x)
    output_rank = list(map(lambda y: 7 - RANKS.index(str(y)) + 1, RANKS))
...
def get_coordinates(move: str):
    return FILES.index(move[0]) * 100 + 50, (7 - RANKS.index(move[1])) * 100 + 50
